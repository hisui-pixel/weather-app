...
export default function Home() {
  ...
  const UserFavoriteCity = favoriteCity.map((city) => (
    <FavoriteCityCard
      key={city.id}
      cityId={city.id}
    ...
    <>
      <div
        className={`min-h-screen ${
          weather ? getBackgroundClass(weather.weather[0].main) : "bg-gray-100"
        }`}
      >
        <div className="flex flex-col items-center text-center w-full mt-16">
          ...
          {weather && !isLoading && (
            <>
              <div
                className="main_weather flex flex-col items-center text-center
              bg-gray-300/50 w-10/12 sm:w-3/4 md:w-1/2 p-4 sm:p-8 rounded-2xl"
              >
                <div className="flex-1">
                  <FavoriteButton onAdd={() => addFavoriteCity(weather.name)} />
                </div>
                ...
                <div className="flex gap-4 w-full m-8 justify-center">
                  <ul className="flex flex-col gap-4 w-8/10">
                    {getDailyForecast(forecast).map((item) => (
                      <li
                        key={item.dt}
                        className="flex flex-col text-shadow-[2px_2px_4px_rgba(0,0,0,0.5)]"
                      >
